...
const HeroSection = () => {
  ...
  return (
    <div className='w-full flex xl:flex-row flex-col gap-10 justify-center min-h-screen max-container'>
      <div className='relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28'>
        ...
        <ul className='flex flex-wrap gap-16 w-full mt-20 items-center justify-starts'>
          {statistics.map((item) => (
            <li key={item.label}>
              <span className='font-palanquin font-bold text-4xl'>
                {item.value}
              </span>
              <p className='font-monsterrat leading-7 text-slate-gray'>
                {item.label}
              </p>
            </li>
          ))}
